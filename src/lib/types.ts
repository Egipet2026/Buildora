...
/**
 * Why a page is showing a built-in template instead of a generated answer.
 *
 * The difference matters to whoever has to fix it: one is a missing setting,
 * the other is a key that exists but did not work — a wrong key, an account
 * out of credit, a rate limit. Telling someone "no provider is configured"
 * when they have just configured one sends them to look in the wrong place.
 */
export type OfflineReason = "not_configured" | "call_failed";
...
export interface BusinessPlan {
  ...
  /** Why the template was used. Absent when the model produced the plan. */
...
export interface MarketResearch {
  ...
  generated_offline: boolean;
  /** Why the template was used. Absent when the model produced it. */
  offline_reason?: OfflineReason;
}
